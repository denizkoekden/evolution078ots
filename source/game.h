...
public:
  Game();
	...
	/**
	  ...
	  */
	bool internalPlaceCreature(const Position& pos, Creature* creature, bool force = false);
	bool placeCreature(const Position& pos, Creature* creature, bool forced = false);
	...
	bool playerSetFightModes(Player* player, fightMode_t fightMode, chaseMode_t chaseMode, uint8_t safeMode);
	...
	bool savePremium(Player* player, unsigned long premiumTime, bool serverSave = false);
	bool checkPremium(Player* player);
	#endif
