	...
	if(Player* player = creature->getPlayer()){
		if(params.valueCallback){
			...
			switch(formulaType){
                ...
                case FORMULA_SKILL:
                    max = (int32_t)(player->getWeaponDamage());
                    min = (int32_t)(max);
                    break;
...
ReturnValue Combat::canDoCombat(Creature* attacker, Creature* target)
{
	if(attacker && attacker->getPlayer() && target->getPlayer()){
		if(attacker->getPlayer()->getAccessLevel() >= ACCESS_PROTECT){
			return RET_NOERROR;
		}
		else if(target->getPlayer()->getAccessLevel() >= ACCESS_PROTECT){
			return RET_YOUMAYNOTATTACKTHISPLAYER;
		}
		else if(target->getPlayer()->getVocationId() == 0){
			return RET_YOUMAYNOTATTACKTHISPLAYER;
		}
		#ifdef __XID_PVP_FEATURES__
		else if(g_config.getString(ConfigManager::OUTFIT_DAMAGE) == "no"){
			if(attacker->currentOutfit.lookFeet == target->currentOutfit.lookFeet){
				return RET_YOUMAYNOTATTACKTHISPLAYER;
			}
		}
		#endif
		#ifdef __XID_PROTECTION_SYSTEM__
		else if(target->getPlayer()->isProtected(attacker->getPlayer())){
			return RET_YOUMAYNOTATTACKTHISPLAYER;
		}
		#endif
	}
	...
	if(g_game.getWorldType() == WORLD_TYPE_NO_PVP){
		if(attacker && (attacker->getPlayer() || (attacker->getMaster()
			&& attacker->getMaster()->getPlayer()))){
			if(target->getPlayer()){
				return RET_YOUMAYNOTATTACKTHISPLAYER;
			}
...
bool Combat::CombatHealthFunc(Creature* caster, Creature* target, const CombatParams& params, void* data)
{
	...
	if(healthChange < 0){
		if(caster->getPlayer() && target->getPlayer()){
