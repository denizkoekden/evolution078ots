...
bool NetworkMessage::ReadFromSocket(SOCKET socket)
{
	...
	if(m_encryptionEnabled){
		if(!m_keyset){
			addConsoleMessage("Error! Could not find key.");
			return false;
		}
		if((m_MsgSize - 2) % 8 != 0){
			addConsoleMessage("Error! Invalid encrypted message size.");
			return false;
		}
		XTEA_decrypt();
		int tmp = GetU16();
		if(tmp > m_MsgSize - 4){
			addConsoleMessage("Error! Invalid unencrypted message size.");
			return false;
		}
		m_MsgSize = tmp;
	}
...
bool NetworkMessage::RSA_encrypt()
{
	...
	if(newPos < 0){
		addConsoleMessage("Error! Invalid packet size.");
		return false;
	}
	
	if(!m_RSA){
		addConsoleMessage("Error! Could not find RSA key.");
		return false;
	}
